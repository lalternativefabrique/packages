...
**Streaming servers.** Every request offers `Accept:
application/x-lalter-audio-frames`. A server that synthesizes as it goes can
answer with the reading as length-prefixed pieces — a big-endian `uint32` byte
count then that many bytes, each piece a complete audio file — and
