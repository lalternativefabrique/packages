...
// filePart carries a document — a PDF, typically — rather than a picture of
// one. Providers that accept PDFs render the pages themselves, so nothing has
// to be rasterised here; sending a PDF as image_url is refused.
...
type fileData struct {
	...
	// FileData is a data URL, e.g. data:application/pdf;base64,...
...
func (d *Describer) describeEncoded(ctx context.Context, dataURI, question string) (string, error) {
	return d.ask(ctx, dataURI, question, systemPrompt, 2000)
}

// contentPart chooses how the payload travels: a PDF goes as a file the
// provider renders itself, anything else as an image.
func contentPart(dataURI string) any {
	if strings.HasPrefix(dataURI, "data:application/pdf") {
		return filePart{Type: "file", File: fileData{Filename: "document.pdf", FileData: dataURI}}
	}
	return imagePart{Type: "image_url", ImageURL: imageURL{URL: dataURI, Detail: "high"}}
}

func (d *Describer) ask(ctx context.Context, dataURI, question, system string, maxTokens int) (string, error) {
