...
// ReadDocument sends a document — a PDF, typically — for the provider to
// render and read itself. question carries the caller's own instructions, and
// systemPrompt is not used: transcribing a paper and describing a screenshot
// for a coding agent are different jobs.
//
// maxTokens bounds the answer. A paper transcribed in full runs far past what
// a screenshot description needs, so the caller sets it.
func (d *Describer) ReadDocument(ctx context.Context, data []byte, mime, question string, maxTokens int) (string, error) {
	if mime == "" {
	...
	return d.ask(ctx, dataURI, question, question, maxTokens)
}
